...
pub type ActionFunc = fn (actor_id: &str, origin: Vec2, target: Vec2, damage: f32);

fn projectile_action(actor_id: &str, origin: Vec2, target: Vec2, damage: f32) {
    let mut projectiles = scene::find_node_by_type::<Projectiles>().unwrap();
    projectiles.spawn(actor_id, damage, color::YELLOW, 2.0, origin, target, 15.0, 10.0, 1.0);
}

fn magic_sphere_action(actor_id: &str, origin: Vec2, target: Vec2, damage: f32) {
    let mut projectiles = scene::find_node_by_type::<Projectiles>().unwrap();
    projectiles.spawn(actor_id, damage, color::BLUE, 100.0, origin, target, 2.0, 0.0, 2.0);
}
