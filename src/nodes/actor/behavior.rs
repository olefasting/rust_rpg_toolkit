        ...
        if params.attackers.len() > 0 {
                ...
                if let Some(attacker) = scene::try_get_node(attacker) {
                    if position.distance(attacker.body.position) <= stats.view_distance * 0.9 {
                        match params.aggression {
                            ActorAggression::Passive => return Box::new(FleeMode::new(attacker.handle())),
                            _ => return Box::new(AttackMode::new(attacker.handle())),
                        }
                    }
                }
            }
            ...
            'actor: for actor in enemies {
                ...
                if position.distance(actor.body.position) <= stats.view_distance {
                    return Box::new(AttackMode::new(actor.handle()));
                }
            }
        } else if params.aggression == ActorAggression::Passive {
            'actor: for actor in scene::find_nodes_by_type::<Actor>() {
                for faction in &actor.factions {
                    if factions.contains(faction) {
                        continue 'actor;
                    }
                ...
                if position.distance(actor.body.position) <= stats.view_distance {
                    return Box::new(FleeMode::new(actor.handle()));
                }
            }
        } else {
            if params.is_on_guard {
                ...
                for actor in noisy_actors {
                    ...
                    if distance <= actor.noise_level.to_range() && distance >= stats.view_distance * 0.9 {
                        return Box::new(InvestigateMode::new(actor.body.position));
                    }
                }
            }
...
impl AttackMode {
    pub fn new(target: Handle<Actor>) -> Self {
        AttackMode {
            target,
            path: None,
        }
...
impl FleeMode {
    pub fn new(from: Handle<Actor>) -> Self {
        FleeMode {
            from,
        }
    }
